...
@Component({
  selector: 'k-bookmark-detail-experience',
  templateUrl: './bookmark-detail-experience.component.html',
  styleUrls: ['./bookmark-detail-experience.component.scss']
})
export class BookmarkDetailExperienceComponent implements OnInit {
  bookmark_lists: any;
                ...
                if (!!rs.bookmark_lists) {
                  ...
                  if (bookmarkData != undefined) {
                    this.listItems = bookmarkData.bookmark_items.filter(
                      x => x.type.indexOf('experiences') > -1
                    );
  ...
  getBookmarkByAlias(page: number = 0) {
    ...
    console.log('[getBookmarkByAlias] listQuery experiences: ', listQuery);

    // Get from Offline Storage First
    const currentList = this.bookmarkService.getItemFromListBookmark(
      this.alias
    );
    console.log('[getBookmarkByAlias] currentList experiences: ', currentList);

    if (!!currentList && currentList.listItems) {
      this.bookmark_lists = currentList.listItems.filter(
        x => x.type === 'experiences'
      );
    ...
    console.log(
      '[getBookmarkByAlias] bookmark_lists experiences offline: ',
      this.bookmark_lists
    );
  ...
  setPage(page: number) {
    ...
    console.log('[Bookmark Detail Experience] setPage : ', this.pager);
  }
  ...
  unBookMarkItem(item: SpotlightItemData) {
    // console.log('[unBookMarkItem] item', item);
    ...
    this.bookmarkService.postUpdateItemToBookmarkList(postData).subscribe(
      rs => {
        // console.log('postUpdateItemToBookmarkList ', rs);
        if (!!rs.success) {
